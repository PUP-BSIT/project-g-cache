...
public interface SpringPomodoroSessionJpaRepository extends JpaRepository<PomodoroSession, Long> {
    ...
    @Query("select s from PomodoroSession s where s.activity.user.id=:userId and s.status = :status")
    List<PomodoroSession> findCompletedByUserId(@Param("userId") Long userId, @Param("status") SessionStatus status);

    @Query("select s from PomodoroSession s where s.activity.user.id=:userId and s.status = :status order by s.completedAt desc")
